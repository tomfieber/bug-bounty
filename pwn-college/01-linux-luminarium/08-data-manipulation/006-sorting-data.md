...
```bash
cat /challenge/flags.txt | sort -r
```

Can also get this by itself by using `head` or `tail`.

```bash
# Using head. Note the reverse sort order
cat /challenge/flags.txt | sort -r | head -n 1
