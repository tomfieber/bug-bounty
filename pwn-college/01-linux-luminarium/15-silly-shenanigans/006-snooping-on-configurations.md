Even without making mistakes, users might inadvertently leave themselves at risk. For example, many files in a typical user's home directory are _world-readable by default_, despite frequently being used to store sensitive information. Believe it or not, your `.bashrc` is world-readable unless you explicitly change it!
...
---
```bash
FLAG_GETTER_API_KEY=sk-962219117
```

Then run `flag_getter`

```bash
flag_getter --key sk-962219117
```
